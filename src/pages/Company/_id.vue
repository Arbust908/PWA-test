<template>
    <Layout>
        <header class="flex flex-col md:flex-row md:justify-between items-center md:mb-4">
            <h1 class="font-bold text-gray-900 text-xl self-start mb-3 md:mb-0">Cliente - {{ id }}</h1>
        </header>
...
export default {
    ...
    setup() {
        ...
        const clients: Array<Client> = JSON.parse(JSON.stringify(store.state.client.all));
        ...
        const update = async() => {
          ...
          let client = await axios
            .put(`${api}/company/${currentClient.id}`, clientToUpdate)
            .catch((err) => {
              console.log(err);
            })
            .then((res) => {
          ...
          let companyRepresentative = await axios
            .put(`${api}/companyRepresentative/${currentClient.companyRepresentative.id}`, clientToUpdate.companyRepresentative)
            .catch((err) => {
              console.log(err);
            })
            .then((res) => {
