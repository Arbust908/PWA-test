        ...
        <div class="grid gap-8 grid-cols-2 relative">
            <section class="panel col-span-full">
                <StageTable>
                    ...
                    <template #body>
                        <StageSheetStage
                            v-if="currentStageSheet && currentStageSheet.stages"
                            :pos="currentStageSheet.stages.length + 1"
                            :stage="currentStage"
                            :editing="currentStage.id"
                            @update:stage="currentStage = $event"
                        />
                    </template>
                </StageTable>
            </section>
        ...
        setup() {
            ...
            const setBox = (slotPos: number) => {
                ...
                if (selectedCradle.value) {
                    ...
                    const isFreeSlot =
                        selectedCradle.value.slots[slotPos].boxId == null ||
                        selectedCradle.value.slots[slotPos].boxId == '';

                    if (isValidBox && isFreeSlot) {
                        selectedCradle.value.slots[slotPos].category = chosenBox.value.category;
            ...
            const isFull = computed(() => {
                const noZeroSandTypeNull =
                    (currentStage.sandId1 !== null && currentStage.sandId1 !== -1) ||
                    (currentStage.sandId2 !== null && currentStage.sandId2 !== -1) ||
                    (currentStage.sandId3 !== null && currentStage.sandId3 !== -1);
                const noZeroSandTypeZero =
                    currentStage.quantity1 !== 0 || currentStage.quantity2 !== 0 || currentStage.quantity3 !== 0;

                return !!(noZeroSandTypeNull && noZeroSandTypeZero);
            });
            const save = (): void => {
