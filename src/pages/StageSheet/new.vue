<template>
    <Layout class="relative">
        <header class="flex flex-col md:flex-row md:justify-between items-center md:mb-4">
            <h2 class="font-bold text-gray-900 text-2xl self-start mb-3 md:mb-0">Stage sheet</h2>
        </header>
...
<script lang="ts">
    ...
    import { Cradle, PurchaseOrder, Box } from '@/interfaces/sandflow';
        ...
        setup() {
            ...
            watch(currentStageSheet, (newVal) => {
                if (newVal && newVal.cradleId && newVal.cradleId !== -1) {
                    ...
                    selectedCradle.value = cradles.value.find((cradle) => cradle.id == newVal.cradleId);
            ...
            const duplicateStage = (stage) => {
                const lastStage = currentStageSheet.stages[currentStageSheet.stages.length - 1];
            ...
            const deleteStage = (stage) => {
                ...
                currentStageSheet.stages = currentStageSheet.stages.filter((s) => s.id !== stageId);
            };
            // :: WAREHOUSE
            const warehouse = ref();
            ...
            watch(clientPitComboSelected, (value) => {
                if (value) {
                    getPurchaseOrder(currentStageSheet.companyId, currentStageSheet.pitId);
            ...
            watchEffect(() => {
                if (currentStageSheet.companyId !== -1 && currentStageSheet.pitId !== -1) {
            ...
            const ableCradles = (clientId: number, pitId: number) => {
                ...
                console.log(lasWO.operativeCradle, lasWO.operativeCradleId, lasWO.backupCradle, lasWO.backupCradleId);
            ...
            const isFull = computed(() => {
                const noZeroSandTypeNull =
                    (currentStageSheet.stages[0].sandId1 !== null && currentStageSheet.stages[0].sandId1 !== -1) ||
                    (currentStageSheet.stages[0].sandId2 !== null && currentStageSheet.stages[0].sandId2 !== -1) ||
                    (currentStageSheet.stages[0].sandId3 !== null && currentStageSheet.stages[0].sandId3 !== -1);
            ...
            const save = (): void => {
                ...
                const { data } = useAxios('/stageSheet', { method: 'POST', data: newStageSheet }, instance);
                // post new workshop
                ...
                watch(data, (newVal) => {
                    if (newVal && newVal.data) {
                        ...
                        const { data } = useAxios('/sandStage', { method: 'POST', data: stage }, instance);
                        watch(data, (newVal) => {
                            if (newVal && newVal.data) {
