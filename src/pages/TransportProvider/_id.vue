<template>
  <Layout>
    <header class="flex flex-col md:flex-row md:justify-between items-center md:mb-4">
      <h1 class="font-bold text-gray-900 text-xl self-start mb-3 md:mb-0">Proveedor de Transporte - ID {{id}}</h1>
    </header>
      ...
      <form method="POST" action="/" class="p-4 w-full flex flex-col lg:flex-row">
        <fieldset class="flex flex-col  w-full">
          <div class="input-block p-4">
            ...
            <div class="mt-1">
              <input v-model="name" class="w-full rounded-md shadow" name="name" type="text" placeholder="Nombre" />
            </div>
          </div>
          <div class="input-block p-4">
            <label for="providerNotificationId" class=""> ProviderNotificationID </label>
            <div class="mt-1">
              <select name="providerNotificationId" v-model="providerNotificationId" class="w-full rounded-md shadow">
          ...
          <div class="input-block p-4">
            ...
            <div class="mt-1">
              <div class="mt-1">
                <input v-model="amount" class="w-full rounded-md shadow" name="amount" type="text" placeholder="Cantidad de cajas" />
              </div>
            </div>
          </div>
          <div class="input-block p-4">
...
export default {
  ...
  setup() {
    ...
    const transportProviders: Array<TransportProvider> = JSON.parse(JSON.stringify(store.state.transportProviders.all));
    ...
    onMounted(async () => {
      ...
      providerNotificationId.value = currentTransportProvider.providerNotificationId
