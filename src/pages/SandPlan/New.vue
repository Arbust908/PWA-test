  ...
  <Layout>
    ...
    <section>
      <form method="POST" action="/" class="p-4 flex flex-col gap-4">
        <fieldset class="py-2 w-full max-w-md grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          <label class="col-span-1" for="sandProvider">
            <span>Cliente</span>
            <input
              v-model="pit"
              class="input"
  ...
  setup() {
    ...
    const isFull = computed(() => {
      return (
        pit.value != '' &&
        stages.value >= 1 &&
        sandStages.value.every((stage) => stage.quantity > 0) &&
        sandStages.value.every((stage) => stage.sandType !== '')
      );
    });
