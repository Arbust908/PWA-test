...
<script lang="ts">
  import { ref, Ref, reactive, computed, ComputedRef, toRaw, toRefs } from 'vue';
...
export default {
  ...
  setup() {
    ...
    const { sandPlan} = useState(['sandPlan']);
    const all = JSON.parse(JSON.stringify(sandPlan.value.all));
    const currentSandPlan = all.find((sandPlan) => {
      return sandPlan.id == route.params.id;
    });

    const {
      pit,
      sandStages,
    } = toRefs(reactive(currentSandPlan));

    console.log(sandStages);
    console.log(pit);
    const defaultStage = {
      id: 0,
      number: 1,
