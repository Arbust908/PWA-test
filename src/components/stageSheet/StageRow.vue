<template>
    <tr>
        <td class="text-gray-500 px-3 py-4 whitespace-nowrap text-lg">{{ pos }} - 40</td>
        <template v-if="editing == Number(stage.id)" :key="stage.id">
            <SandTypeAmount
                :sand-id="stage.sandId1 || -1"
        ...
        <td class="text-gray-500 px-3 py-4 whitespace-nowrap font-bold text-center">{{ totalWheight }}t</td>
    </tr>
</template>
    ...
    export default defineComponent({
        ...
        setup(props, { emit }) {
            ...
            const totalWheight = computed(() => {
                return stage.value.quantity1 + stage.value.quantity2 + stage.value.quantity3 || 0;
            });
